...
areas = 'formsci~ Areas of Study|areas.html~ .Mathematics~ Arithmetic|arithmetic.html~ Algebra|algebra.html~ Geometry|geometry.html~ Probability & Statistics|probstats.html~ Discrete Mathematics|discrete-math.html~ Differential Calculus|calc-diff.html~ Integral Calculus|calc-int.html~ Multivariable Calculus|calc-multi.html~ Differential Equations|diffeq.html~ Linear Algebra|linear-algebra.html~ Combinatorics|combinatorics.html~ '
...
;
chemistry = 'natsci~ Chemistry~ .Fundamentals~ SI Units|si-units.html~ Significant Figures|sig-figs.html~ Scalars &amp; Vectors|scalars-vectors.html~ Scientific Method|scientific-method.html~ Periodic Table~ '
+ '.Atoms~ Moles~ Coulomb\'s Law~ Bohr Atom~ Photoelectron Spectroscopy~ Electron Configuration~ Periodic Trends~ Pauli Exclusion Principle~ '
+ '.Molecules~ Ionic & Metallic Bonds~ Covalent Bonds~ Lewis Structures~ '
+ '.Intermolecular Forces~ Polarity~ Intermolecular Forces~ Solution Separation~ Kinetic Molecular Theory~ Maxwell-Boltzmann Diagrams~ Ideal Gases~ Electromagnetic Spectrum~ Beer\'s Law~ '
+ '.Chemical Reactions~ Chemical Reactions~ Balacing Chemical Equations~ Gravimetric Analysis~ Redox Reactions~ Acid-Base Reactions~ '
+ '.Kinetics~ Rate Law|rate-law.html~ Collision Theory~ Reaction Mechanisms~ Catalysts~ '
+ '.Equilibrium~ Equilibrium Constant~ RICE Tables|rice-tables.html~ Le Ch&acirc;telier\'s Principle~ Solubility~ '
+ '.Acids &amp; Bases~ pH~ Acid Strength~ Neutralization Reactions~ Buffers~ Titration~ '
+ '.Thermodynamics~ Heat~ Temperature~ Enthalpy~ Energy Diagrams~ Enthalpy of Formation~ Bond Energy~ Hess\'s Law~ Enthalpy of Solution~ Phase Changes~ Calorimetry~ Heating Curves~ Free Energy Change~ Galvanic Cells~ Electrolytic Cells~ Gibb\'s Free Energy~ '
;
...
physClassical = 'natsci~ Classical Physics~ .Fundamentals~ SI Units|si-units.html~ Significant Figures|sig-figs.html~ Scalars &amp; Vectors|scalars-vectors.html~ Scientific Method|scientific-method.html~ '
    ...
    + '.Phases of Matter~ Phases of Matter~ Moles~ Thermal Expansion~ Phase Changes~ Ideal Gases~ Ideal-Gas Processes~ '
